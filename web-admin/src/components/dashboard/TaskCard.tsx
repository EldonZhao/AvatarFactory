...
export function TaskCard({ task, index = 0 }: TaskCardProps) {
  const schedule = task.schedule || task.cron_expression;
  let scheduleDesc = schedule;
  try {
    scheduleDesc = cronstrue.toString(schedule, { locale: 'zh_CN', use24HourTimeFormat: true });
  } catch {
    // Keep original if parsing fails
  }

  const getTypeIcon = () => {
    if (task.task_type.includes('discovery') || task.task_type.includes('scan')) {
      return (
    ...
    if (task.task_type.includes('content')) {
      return (
        <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between mb-1">
                <h3 className="font-medium truncate">{task.name || task.task_type}</h3>
              ...
              <div className="flex items-center gap-3 text-xs text-[hsl(var(--muted-foreground))]">
                {task.run_count !== undefined && <span>运行 {task.run_count} 次</span>}
                {task.last_run && (
